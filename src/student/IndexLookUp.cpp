...
IndexLookUp::IndexLookUp(std::shared_ptr<SimpleGraph>& index,QueryEdge queryEdge, bool reversed) :
        PhysicalOperator(nullptr, nullptr,ANY,LOOKUP),
...
IteratorReachable IndexLookUp::reachable(Node s) {
    switch(resultSorted){
        ...
        case ANY          :
        case NOT_SORTED   : //go in SourceSorted
        case SOURCE_SORTED: return (!reversed)?index->targetsReachable(queryEdge.label,s):index->sourcesReachable(queryEdge.label,s);
    }
}
